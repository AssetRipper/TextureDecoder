namespace AssetRipper.TextureDecoder.Tests
{
	public sealed class AtcTests
	{
		...
		{
			ReadOnlySpan<byte> data = File.ReadAllBytes(PathConstants.AtcTestProjectRootFolder + "test.atc_rgb4");
		...
		{
			ReadOnlySpan<byte> data = File.ReadAllBytes(PathConstants.AtcTestProjectRootFolder + "test.atc_rgba8");
