namespace AssetRipper.TextureDecoder.Tests
{
	public sealed class EtcTests
	{
		...
		{
			ReadOnlySpan<byte> data = File.ReadAllBytes(PathConstants.EtcTestProjectRootFolder + "test.etc");
		...
		{
			ReadOnlySpan<byte> data = File.ReadAllBytes(PathConstants.EtcTestProjectRootFolder + "test.etc2");
			int totalBytesRead = 0;
			foreach (int size in new int[] { 256, 128, 64, 32, 16, 8, 4, 2, 1 }) //mip maps
			{
		...
		{
			ReadOnlySpan<byte> data = File.ReadAllBytes(PathConstants.EtcTestProjectRootFolder + "test.etc2a1");
			int totalBytesRead = 0;
			foreach (int size in new int[] { 256, 128, 64, 32, 16, 8, 4, 2, 1 }) //mip maps
			{
		...
		{
			ReadOnlySpan<byte> data = File.ReadAllBytes(PathConstants.EtcTestProjectRootFolder + "test.etc2a8");
			int totalBytesRead = 0;
			foreach (int size in new int[] { 256, 128, 64, 32, 16, 8, 4, 2, 1 }) //mip maps
			{
		...
		{
			ReadOnlySpan<byte> data = File.ReadAllBytes(PathConstants.EtcTestProjectRootFolder + "test.eac_r");
			int totalBytesRead = 0;
			foreach (int size in new int[] { 256, 128, 64, 32, 16, 8, 4, 2, 1 }) //mip maps
			{
		...
		{
			ReadOnlySpan<byte> data = File.ReadAllBytes(PathConstants.EtcTestProjectRootFolder + "test.eac_rg");
			int totalBytesRead = 0;
			foreach (int size in new int[] { 256, 128, 64, 32, 16, 8, 4, 2, 1 }) //mip maps
			{
